...
def main():
    ...
    '''.format(pa, version, os.environ.get('api-issuer'), os.environ.get('api-key')))
                ...
                os.system('dpkg-deb -R reachdesk.deb tmpdeb')
                os.system('mkdir -p tmpdeb/usr/share/reachdesk/files/systemd/')
                os.system(
                    'cp res/reachdesk.service tmpdeb/usr/share/reachdesk/files/systemd/')
                os.system(
                    'cp res/128x128@2x.png tmpdeb/usr/share/reachdesk/files/rustdesk.png')
                os.system(
                    'cp res/reachdesk.desktop tmpdeb/usr/share/applications/rustdesk.desktop')
                os.system(
                    'cp res/reachdesk-link.desktop tmpdeb/usr/share/applications/rustdesk-link.desktop')
                os.system('cp -a res/DEBIAN/* tmpdeb/DEBIAN/')
                os.system('strip tmpdeb/usr/bin/reachdesk')
                os.system('mkdir -p tmpdeb/usr/lib/reachdesk')
                os.system('mv tmpdeb/usr/bin/reachdesk tmpdeb/usr/lib/reachdesk/')
                os.system('cp libsciter-gtk.so tmpdeb/usr/lib/reachdesk/')
                md5_file('usr/share/reachdesk/files/systemd/reachdesk.service')
                md5_file('usr/lib/reachdesk/libsciter-gtk.so')
                os.system('dpkg-deb -b tmpdeb reachdesk.deb; /bin/rm -rf tmpdeb/')
